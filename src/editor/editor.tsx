...
import { action, autorun, makeAutoObservable, observable, reaction, runInAction, toJS } from "mobx";
...
const Editor = observer(() => {
    ...
    useEffect(() => {
        const dispose = autorun(() => {
            ...
            if (hasLayers) {
                previewWorker.renderPreview({ layers: Array.from(layers.values()).map(layer => toJS(layer)), slice: toJS(slice) });
            }
        });
        return () => dispose();
    }, [store.previewHash])
    ...
    const uploadFile = action((files: FileList | undefined) => {
        ...
        Array.from(files).map(file => {
            ...
            store.name === '' && (store.name = files[0].name.split('.').shift() || 'emoji');
            const dispose = autorun(() => {
                const { flipX, flipY, resize } = layer.edits;
                const { x, y } = store.slice;
                layerWorker.renderLayer({ id, file, edits: { flipX, flipY, resize }, slice: { x, y } });
            });
    ...
    return <div className="flex flex-col h-full gap-6">
        <div className="grow flex flex-col gap-6">
            {Array.from(layers.entries()).map(([id, layer]) => {
                return <LayerEditor key={id} slice={slice} layer={layer} onDelete={onDelete} />
            })}
            <div>
                <FileInput onFileUpload={(files) => uploadFile(files)} label={hasLayers ? 'Upload another image' : 'Upload an image to get started'} />
            </div>
        ...
        <div className="grid gap-6 grid-flow-col justify-between items-center">
            ...
            <Slice valueX={x} valueY={y} onChangeX={onChangeSliceX} onChangeY={onChangeSliceY} />
...
const LayerEditor = observer(({ slice, layer, onDelete }: { slice: Slice, layer: LayerState, onDelete(id: string): void }) => {
    ...
    return <div className="border border-slate-500 rounded p-6">
        <div className="flex w-full gap-6">
            ...
            <div className="grow grid grid-flow-row gap-4">
                ...
                <Button onClick={() => onDelete(id)}><Text weight="bold" align="center">Delete</Text></Button>
            </div>
        </div>
    </div>
});
...
// TODO; Scale properly
const EmojiPreview = ({ images, slice }: { images: string[], slice: Slice }) => {
    ...
    return <div className="relative">
        <div className={classNames("grid grid-flow-row items-center", gap)}>
            {mapFromSlice(images, slice).map((row, rowIndex) => {
                return <div className={classNames("grid grid-flow-col items-center", gap)} key={rowIndex}>
                    {row.map((col, colIndex) => <div key={colIndex} className="aspect-square relative">
                        <Image src={col} />
                    </div>)}
                </div>
            })}
